...
class RecommendationEngine:
    def __init__(self):
        ...
        try:
            # This is the critical change. We now read the database URL
            # from an environment variable named 'DATABASE_URL'.
            # This makes our code portable and secure.
            db_url = os.environ.get('DATABASE_URL')

            if not db_url:
                raise ValueError("DATABASE_URL environment variable not set.")

            # SQLAlchemy automatically knows how to handle postgresql:// URLs
            self.db_engine = create_engine(db_url)

            # Load the full restaurants table into memory once for fast distance calculations
    ...
    def get_recommendations(self, user_id, n_recommendations=5, distance_limit_km=5):
        ...
        # --- 1. Get User Location & Cluster from DB ---
        query = f"""
        ...
        if user_data.empty: return None # User not found

        user_lat, user_lon, user_cluster_id = user_data.iloc[0]
        # Handle cases where a user might not have a cluster
        ...
        if user_cluster_id in [-1, 46]: # Non-Personalized for Noise or Mainstream
            recommendations = nearby_restaurants.sort_values(by='rating', ascending=False)
        else: # Personalized for Niche Tribes
